...
def languages():
    with open('./langs.txt', 'w', encoding='utf-8') as json_file:
        json_file.write(" language --> language-code \n")
    langs = []
    ...
    if languages:
        for code, name in languages.items():
            langs.append(name)
            with open('./langs.txt', 'a', encoding='utf-8') as json_file:
                json_file.write(str(code) + " --> " + str(name) + "\n")
